...
export default function ExamDetailPage() {
    ...
    const { getItem } = useExams()
    const { getItem: getPatient } = usePatients()
    ...
    useEffect(() => {
        const loadExamData = async () => {
            if (params.id) {
                const examData = await getItem(params.id as string)
                if (examData) {
                    setExam(examData)
                    // Cargar datos del paciente
                    if (examData.patient_id) {
                        const patientData = await getPatient(examData.patient_id)
